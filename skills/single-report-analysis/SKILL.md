...
5. Final answer MUST include inline footnotes like `[^interview_001§L123]` or `[^interview_001§L100-L150]` and an end section `## 参考文献摘录`. If you used `task(report_analyst)`, rewrite its findings/references into that final answer (keep line footnotes); do not end with only a wrap-up paragraph.
